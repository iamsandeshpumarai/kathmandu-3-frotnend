...
const UserDetailsModal = ({ isOpen, onClose, title, users }) => {
  ...
  const handlePrintUsers = () => {
    ...
    printWindow.document.write(`<h3>${title} (${users.length})</h3>`);
      ...
      <div className="bg-white rounded-3xl w-full max-w-2xl max-h-[85vh] overflow-hidden shadow-2xl flex flex-col scale-in-center">
        ...
        <div className="p-4 border-t border-gray-100 bg-gray-50 flex justify-end gap-4">
          <button 
            ...
            className="px-8 py-2.5 bg-green-600 text-white rounded-xl font-bold text-sm hover:bg-green-700 transition-all shadow-md active:scale-95"
          >
            प्रिन्ट गर्नुहोस्
          </button>
          <button 
